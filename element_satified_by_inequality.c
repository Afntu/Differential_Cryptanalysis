...
#include<stdio.h>
#include<stdlib.h>
void checking(int **x,int **y,int **a,int *p,int i)
{
...
int main()
{
	...
	for(int j = 0; j < n; j++)
	{
		x[j] = (int*)malloc(3*sizeof(int));
		y[j] = (int*)malloc(3*sizeof(int));
	}
	for(int j = 0; j < n; j++)
	{
		a[j] = (int*)malloc(6*sizeof(int));
	}
	...
	for(int i = 0;i<n;i++)
	{
		...
		checking(x,y,a,&p[0]			,i);    // equivalent checking(x,y,a,&p[0],i);
	} 
	for(int i = 0;i<n;i++)
	{
		if(p[i] == 1)
